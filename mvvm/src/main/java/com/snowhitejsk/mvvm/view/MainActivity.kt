package com.mrhi2022.mvvm.view

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import androidx.databinding.DataBindingUtil
import androidx.databinding.ObservableField
import com.snowhitejsk.mvvm.R
import com.snowhitejsk.mvvm.databinding.ActivityMainBinding
import com.snowhitejsk.mvvm.viewmodel.ItemViewModel
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //MVVM의 장점
        //1.MVP의 view와 presenter 처럼 1:1로 대응되어 있지 않기에 뷰가 많아져도 뷰모델이 많아지지않아도 됨
        //2. 사용자 이벤트 처리를 모두 viewModel 역할 클래스에서 하고 있기에 중복되는 코드를 다시 작성하지않고 재사용가능함.
        //3. 데이터 바인디이 되어 있기에 데이터가 변경되었을때 별도의 setText() or notifyDataSetChange()같은 갱신코드가 필요
        //4. Acitivity코드가 가장 간결함.

        //MVVM의 단점
        //1. data binding 레이아웃 설계 작업이 다소 복잡함
        //2. view 처리가 복잡할 수록 결국 viewModel 클래스가 엄청 비대해짐

        //##### android 는 MVVM패턴에 대한 제작을 보다 쉽게 해주기 위해 별도의 지원 클래스 API를 만들어서
        //JetPack 으로 제공하고 있음. ##################
        // *** AAC - Android Architecture Component ****
        // * 대표 클래스 - viewModel, Lifecycle
